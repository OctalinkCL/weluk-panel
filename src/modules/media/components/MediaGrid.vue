    ...
    <div v-else class="grid grid-cols-2 sm:grid-cols-4 gap-3 p-2">
      ...
      <div v-for="item in queue" :key="item.id" class="relative border rounded-lg overflow-hidden">
        ...
        <div class="p-2 flex items-center justify-between gap-1">
          ...
          <Button v-if="item.status === 'error'" size="icon-sm" variant="ghost" class="shrink-0"
      ...
      <div v-for="item in media" :key="item.id"
        class="group relative  rounded-lg overflow-hidden cursor-pointer  transition bg-background shadow-xs hover:shadow-md"
        :class="[
          selectedIds.has(item.id) ? 'ring-3 ring-blue-600' : '',
          mediaIdsInPlaylist.has(item.id) ? 'opacity-60' : '',
        ]" @click="emit('toggle', item.id)">
        <div class="aspect-video bg-muted flex items-center justify-center">
          <img v-if="item.thumbnail_path || item.type === 'image'"
            :src="getMediaPublicUrl(item.thumbnail_path ?? item.storage_path)" loading="lazy"
        ...
        <div v-if="mediaIdsInPlaylist.has(item.id)"
